...
const coreValues = [
  {
    icon: <Leaf className="w-10 h-10 text-primary" />, // Green icon
    title: "Eco-Friendly",
    description: "We use environmentally safe practices and disposal methods."
  },
  {
    icon: <ShieldCheck className="w-10 h-10 text-primary" />, // Green icon
    title: "Reliable Service",
    description: "Count on us for timely and consistent pet waste removal."
  },
  {
    icon: <Smile className="w-10 h-10 text-primary" />, // Green icon
...
export default function AboutSection() {
  return (
    <section id="about" className="min-h-screen flex items-center bg-accent">
      <div className="container py-16 md:py-24">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-accent-foreground">About Valley Pet Waste Management</h2>
          ...
          <div className="space-y-8">
            {coreValues.map((value) => (
              <div key={value.title} className="flex items-start gap-4">
                <div className="flex-shrink-0 p-3 bg-background/50 rounded-full">
                  {value.icon}
                </div>
                <div>
                  <h3 className="text-xl font-semibold text-accent-foreground">{value.title}</h3>
                  <p className="text-accent-foreground/70 mt-1">{value.description}</p>
                </div>
              </div>
            ))}
          </div>
